...
Noxg::MachineGear::MachineGear(rf::Texture _texture, int _level, float _baseSize, float _thickness) : texture{ _texture }, level{ std::max(_level, 1) }, baseSize{ std::abs(_baseSize) }, thickness{ std::abs(_thickness) }
{
